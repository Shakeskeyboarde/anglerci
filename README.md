...
To create a prerelease, include a [prerelease](https://semver.org/#spec-item-9) part in the workspace version. When a workspace has a prerelease version, the following things happen:
...
To enforce prerelease versions (eg. from unprotected branches), pass the `--prerelease` option to Angular CI. It will then refuse to release non-prerelease versions.
